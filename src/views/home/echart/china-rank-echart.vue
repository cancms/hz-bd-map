<template>
    <div class="china-rank-echart w-full h-full">
        <div ref="echartRef" class="w-full h-full"></div>
    </div>
</template>
<script setup name='china-rank-echart' lang="ts">
...
const { chinaRank } = storeToRefs(store)
const setOpetion = () => {
    const nameArray = chinaRank.value.map(item => item.name)
    const valueArray = chinaRank.value.map(item => item.echartValue)
    return {
        grid: {
            left: "18%",
            right: "20%",
...
useEchart(echartRef, () => chinaRank, setOpetion)
