...
const { guangdongRank } = storeToRefs(store)
const setOpetion = () => {
    const nameArray = guangdongRank.value.map(item => item.name)
    const valueArray = guangdongRank.value.map(item => item.echartValue)
    return {
        grid: {
            left: "20%",
            right: "18%",
                    ...
                    <div class="z-tooltip">
                      ...
                      <div class="z-tooltip__main">
                        <div class="z-tooltip__group">
                            <span class="z-tooltip__group--item-label">新增</span>
                            <span class="z-tooltip__group--item-value">${param[0].data.today.confirm}</span>
                        </div>
                        <div class="z-tooltip__group">
                            <span class="z-tooltip__group--item-label">累计</span>
                            <span class="z-tooltip__group--item-value">${param[0].data.total.confirm}</span>
                        </div>
                      </div>
                    </div>
                  `;
...
useEchart(echartRef, () => guangdongRank, setOpetion)
